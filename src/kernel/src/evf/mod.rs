...
impl EvfManager {
    ...
    fn sys_evf_create(self: &Arc<Self>, i: &SysIn) -> Result<SysOut, SysErr> {
        ...
        let (entry, id) =
            objects.alloc::<_, MmapError>(|_| Ok(EventFlag::new(attr, init_pattern)))?;
    ...
    fn sys_evf_open(self: &Arc<Self>, i: &SysIn) -> Result<SysOut, SysErr> {
        ...
        match existing {
            ...
            Some(entry) => {
                ...
                let mut objects = self.proc.objects_mut();
                let (fd_entry, id) = objects.alloc::<_, MmapError>(|_| Ok(flag.clone()))?;
    ...
    fn sys_evf_close(self: &Arc<Self>, i: &SysIn) -> Result<SysOut, SysErr> {
    ...
    fn sys_evf_wait(self: &Arc<Self>, i: &SysIn) -> Result<SysOut, SysErr> {
        let evf: usize = i.args[0].into();
    ...
    fn sys_evf_try_wait(self: &Arc<Self>, i: &SysIn) -> Result<SysOut, SysErr> {
        let evf: usize = i.args[0].into();
    ...
    fn sys_evf_set(self: &Arc<Self>, i: &SysIn) -> Result<SysOut, SysErr> {
    ...
    fn sys_evf_clear(self: &Arc<Self>, i: &SysIn) -> Result<SysOut, SysErr> {
        let evf = i.args[0].into();
        let pattern: u64 = i.args[1].into();
    ...
    fn sys_evf_cancel(self: &Arc<Self>, i: &SysIn) -> Result<SysOut, SysErr> {
        let evf = i.args[0].into();
        let pattern: u64 = i.args[1].into();
