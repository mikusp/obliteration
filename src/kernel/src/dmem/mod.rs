...
use crate::fs::{make_dev, Cdev, CdevSw, DriverFlags, Fs, IoCmd, MakeDev, Mode, OpenFlags};
...
impl DmemManager {
    pub fn new(fs: &Arc<Fs>, mm: &Arc<MemoryManager>, sys: &mut Syscalls) -> Arc<Self> {
        ...
        let dce = Arc::new(CdevSw::new(
            DriverFlags::D_INIT,
            Some(|_, _, _, _| Ok(())),
            None,
            Some(|_, _, _, _| Ok(0.into())),
        ));
    ...
    fn dmem_ioctl(
        ...
        match cmd {
            // dmem size
            a if a == IoCmd::try_from_raw(0x4008800a).unwrap() => {
            ...
            // available dmem
            a if a == IoCmd::try_from_raw(0xc0208016).unwrap() => {
                #[repr(C)]
                struct Args {
            ...
            // allocate dmem
            a if a == IoCmd::try_from_raw(0xc0288001).unwrap() => {
                #[repr(C)]
                struct Args {
                    pub search_start: u64,
                    pub search_end: u64,
            ...
            a if a == IoCmd::try_from_raw(0xc0288011).unwrap() => {
                info!("stubbed dmem_ioctl 0x80288011");
                Ok(0.into())
            }
            a if a == IoCmd::try_from_raw(0x80288012).unwrap() => {
                #[repr(C)]
                struct Args {
