...
impl OsemManager {
    ...
    fn sys_osem_delete(self: &Arc<Self>, i: &SysIn) -> Result<SysOut, SysErr> {
    ...
    fn sys_osem_open(self: &Arc<Self>, i: &SysIn) -> Result<SysOut, SysErr> {
        ...
        match existing {
            ...
            Some(entry) => {
                ...
                let mut objects = self.proc.objects_mut();
                let (fd_entry, id) = objects.alloc::<_, MmapError>(|_| Ok(osem.clone()))?;
    ...
    fn sys_osem_close(self: &Arc<Self>, i: &SysIn) -> Result<SysOut, SysErr> {
    ...
    fn sys_osem_wait(self: &Arc<Self>, i: &SysIn) -> Result<SysOut, SysErr> {
        let sem_id = i.args[0].into();
    ...
    fn sys_osem_post(self: &Arc<Self>, i: &SysIn) -> Result<SysOut, SysErr> {
        let sem_id = i.args[0].into();
