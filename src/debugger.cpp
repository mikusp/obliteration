...
Debugger::Debugger(QProcess* kernel, SymbolResolver* resolver): m_kernel(kernel), t(nullptr), m_symbol_resolver(resolver), m_leave_debuggee_running(false)
{
            ...
            if (WIFSTOPPED(status))
            {
                ...
                else if (WSTOPSIG(status) == SIGSEGV)
                {
                    ...
                    for (;;)
                    {
                        if (QThread::currentThread()->isInterruptionRequested())
                        {
                            if (m_leave_debuggee_running)
                            {
                                if (-1 == ptrace(PTRACE_SYSCALL, ret, 0, SIGSTOP)) {
                                    qDebug() << "sending SIGSTOP to" << ret << "failed with" << strerror(errno);
                                }
                                break;
                            }
                            // int sig = m_leave_debuggee_running ? SIGSTOP : 0;
                            // m_leave_debuggee_running = false;
                ...
                else if (WSTOPSIG(status) == SIGSTOP)
                {
                    qDebug() << "SIGSTOP";
                    qDebug() << "detaching from kernel";
                    if (-1 == ptrace(PTRACE_DETACH, ret, 0, 0))
                        qDebug() << "PTRACE_DETACH failed with" << strerror(errno);
                    // while(1){}
                }
                else
                {
                    qDebug() << "stopped for other reason";
                    qDebug() << "PTRACE_CONT";
...
void Debugger::detachZombie()
{
    m_leave_debuggee_running = true;
    detach();
}
